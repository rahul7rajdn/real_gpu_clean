...
def test_attention(causal=False):
    ...
    torch_self_attention = (
        torch.nn.MultiheadAttention(
            embed_dim=hidden_dim, num_heads=num_heads, bias=False, batch_first=True
        )
        .to(torch.float)
        .to("cuda")
        .eval()
    ...
    with torch.no_grad():
        ...
        if causal:
            attn_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1).bool().to(
                "cuda"
            )
        else:
            attn_mask = None
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--softmax", action="store_true", default=False, help="Test the softmax function"
    )
    args.add_argument(
        "--gemm", action="store_true", default=False, help="Test the GEMM function"
    )
    args.add_argument(
        "--attention", action="store_true", default=False, help="Test the attention function"
    )
